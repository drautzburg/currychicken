...
We define |Time| in a way which respects the way composers think. The
problem statement suggests, that we need a way to express |StartTime|
and |Duration|. We express both as |Rational| numbers.
...
associated values ("payload"), the type has to be |Events ()|.
...
To use |apply| we need a |Behavior (()->[Note])|, because our events
have the type |Events ()|. However, we only have |Behavior Chord|. We
need to |fmap| a function which takes a |Chord| and returns a function
|() -> [Note]|. The function |renderChord| is close, but it returns
Notes right away, instead of a function |()->[Note]|. Converting some
value |a| into a function |()->a| is what |const| does:
